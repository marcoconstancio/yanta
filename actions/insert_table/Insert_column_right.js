        ...
        if (row.cells[table_cell_column_number].tagName.toLowerCase() === 'th') {
            if(this_rowspan === undefined || this_rowspan == 1){
                if(merged_td > 0){
                    ...
                    row.cells[table_cell_column_number].insertAdjacentHTML('afterend','<th></th>');
                }
            }else{
                merged_td = this_rowspan - 1;
                row.cells[table_cell_column_number].insertAdjacentHTML('afterend','<th rowspan="' + this_rowspan + '"></th>');
            }
           ...
           if(this_rowspan === undefined || this_rowspan == 1){
                if(merged_td > 0){
                    ...
                    row.cells[table_cell_column_number].insertAdjacentHTML('afterend','<td></td>');
                }
           }else{
                merged_td = this_rowspan - 1;
                row.cells[table_cell_column_number].insertAdjacentHTML('afterend','<th rowspan="' + this_rowspan + '"></th>');
